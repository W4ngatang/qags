...
@register_model('summerization_transformer')
class SummerizationTransformerModel(FairseqModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--share-decoder-input-output-embed', action='store_true', 
                            help='share decoder input output embed')
    ...
    def build_model(cls, args, task):
        ...
        encoder_embed_tokens = build_embedding(dict, args.encoder_embed_dim, args.encoder_embed_path)
        decoder_embed_tokens = encoder_embed_tokens.decoder.bert.embeddings.word_embeddings
        decoder_embed_positions = encoder_embed_tokens.decoder.bert.embeddings.position_embeddings

        encoder = EmbeddingEncoder(dict, encoder_embed_tokens)
        decoder = TransformerDecoder(args, dict, decoder_embed_tokens, decoder_embed_positions)
        
        return SummerizationTransformerModel(encoder, decoder)
...
class EmbeddingEncoder(FairseqEncoder):
    ...
    def __init__(self, dictionary, embed_tokens):
        ...
        self.padding_idx = embed_tokens.task.dictionary.pad()

    def forward(self, src_tokens, src_lengths, segment):
        # embed tokens and positions
        x, _ = self.embedder(src_tokens, segment, apply_mask=False)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_positions, embed_scale=None, no_encoder_attn=False, left_pad=False,
                 final_norm=True):
        super().__init__(dictionary)
        self.dropout = args.dropout
        self.share_input_output_embed = args.share_decoder_input_output_embed

        input_embed_dim = embed_tokens
        ...
        self.project_in_dim = Linear(input_embed_dim, self.embed_dim,
                                     bias=False) if self.embed_dim != input_embed_dim.weight.size(1) else None
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        position_ids = torch.arange(prev_output_tokens.size(1)).type_as(prev_output_tokens)
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        self.self_attn = MultiheadAttention(
            ...
            pos_emb_type=args.attn_position_embedding_type,
            max_k_pos_emb=args.max_key_position_embedding,
            max_v_pos_emb=args.max_val_position_embedding,
...
@register_model_architecture('summerization_transformer', 'vanilla_small')
...
@register_model_architecture('summerization_transformer', 'summerization_bert')
